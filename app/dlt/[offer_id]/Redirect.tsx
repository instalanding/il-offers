...
const Redirect = ({ data }: any) => {
  ...
  const amazonUrl = encodeURIComponent(data.product_url);
  const buttonType: string = data.cta_type;

  if (buttonType === "amazon") {
    redirectUrl = `${process.env.NEXT_PUBLIC_REDIRECT_SCRIPT_URL}amazon-redirect/?redirect_url=${amazonUrl}&ctatype=${buttonType}`;
  } else {
