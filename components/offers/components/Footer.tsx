...
const Footer: React.FC<{ config: Config, price: Price, checkoutData: Checkout }> = ({ config, price, checkoutData }) => {
    ...
    const handleCheckoutButtonClick = async (e: React.MouseEvent) => {
        try {
            if (checkoutData.checkout_name === "fastrr") {
                handleCheckout(
        ...
        <><input type="hidden" value={checkoutData.store_url} id="sellerDomain" />
            <div className="sticky bottom-0 bg-white">
                <div className="flex flex-col">
                    <div
                        style={{
                            backgroundColor: config?.primaryColor + "7a",
